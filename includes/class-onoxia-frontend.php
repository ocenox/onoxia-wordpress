...
class Onoxia_Frontend {
    ...
    public function inject_widget() {
        ...
        wp_enqueue_script('onoxia-widget', $widget_url, [], null, true); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion

        // Build data attributes as inline script data
        $data = [
            'site' => $this->site_uuid,
        ];
        ...
        // Add data attributes to the script tag via filter
        add_filter('script_loader_tag', function ($tag, $handle) use ($data) {
            if ('onoxia-widget' !== $handle) {
                return $tag;
            }

            $attrs = '';
            $attrs .= ' data-site="' . esc_attr($data['site']) . '"';

            if (!empty($data['tags'])) {
                $attrs .= ' data-tags="' . esc_attr($data['tags']) . '"';
            }

            if (!empty($data['context'])) {
                $attrs .= " data-context='" . esc_attr(wp_json_encode($data['context'])) . "'";
            }
    ...
    private function build_context() {
        ...
        // Logged-in user context (opt-in via admin settings)
        if (get_option('onoxia_user_context') && is_user_logged_in()) {
        ...
        // WooCommerce cart context (opt-in via product context setting)
        if (get_option('onoxia_product_context') && function_exists('WC') && WC()->cart) {
            $cart = WC()->cart;
